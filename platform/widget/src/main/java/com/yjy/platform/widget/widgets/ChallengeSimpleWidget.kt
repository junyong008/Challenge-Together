...
import com.yjy.platform.widget.util.formatGlanceDaysOnly
import dagger.hilt.android.EntryPointAccessors
import kotlinx.coroutines.flow.first
import timber.log.Timber

class ChallengeSimpleWidget : GlanceAppWidget() {
    ...
    private fun ChallengeContent(
        ...
        Column(
            ...
            Text(
                ...
                style = WidgetTypography.bodyMedium.copy(
                    color = WidgetColorScheme.onSurface(),
                ),
            )
            Text(
            ...
            Text(
                text = formatGlanceDaysOnly(
                    seconds = challenge.currentRecordInSeconds,
                    context = context,
                ),
                style = WidgetTypography.displayMedium.copy(
                    color = WidgetColorScheme.onSurface(),
    ...
    companion object {
        ...
        suspend fun updateWidget(context: Context) {
            try {
                val glanceIds = GlanceAppWidgetManager(context).getGlanceIds(ChallengeSimpleWidget::class.java)
                glanceIds.forEach { glanceId ->
                    updateAppWidgetState(context, glanceId) { prefs ->
                        prefs[longPreferencesKey(LAST_UPDATE_KEY)] = System.currentTimeMillis()
                    }

                    ChallengeSimpleWidget().update(context, glanceId)
                }
            } catch (e: Exception) {
